...
구현 시 대시보드는 카드 API나 복습 API를 화면에서 직접 조합하지 않고 Dashboard API를 단일 진입점으로 사용한다.

대시보드의 학습 진입 영역은 `learningQueues` 목록을 기준으로 렌더링한다.

- 현재 MVP1에서는 `FLASH_CARD_REVIEW` 큐를 제공한다.
- 새로운 학습 방식이 추가되면 독립 도메인을 유지한 채 학습 큐 요약을 추가한다.
- 화면은 남은 학습 수, 오늘 완료 수, 학습 시작 동작을 학습 큐 단위로 보여준다.
- 알 수 없는 학습 큐가 반환되어도 대시보드 전체가 깨지지 않도록 기본 표시를 제공한다.
